...
parser.add_argument('-o', '--output', default="ascii", help="the output format", choices=maze.RectGrid.outputs)
...
if m := re.match(r'(\d+)x(\d+)$', args.size):
    ...
    grid = maze.RectGrid(height, width)
    ...
    if mask_filename[-4:] == '.png':
        grid = maze.RectGrid.from_mask_png(mask_filename)
    else:
        grid = maze.RectGrid.from_mask_txt(mask_filename)
else:
